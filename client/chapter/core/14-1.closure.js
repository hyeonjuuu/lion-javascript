...
//1번() -> first(), 2번() -> second ()
// first()();
// second라는 함수 본문이 내뱉어짐
// -> second 라는 함수를 실행시켜 값을 얻으려면-> first()()

// value에 first 실행한 값을 담음 = second 함수
// value() === first()()
const value = first();

// 함수가 실행되어 내보내진게 아님! 본문을 내보낸거임!!
/* first 함수를 실행함과 동시에 second 함수를 내뱉는다.
그 함수를 실행하여 값을 얻으려면 first()() => currying function
그래서 그 curring function인 first()() 를 value에 담는다. */
...
// 함수의 본문이 result에 할당
// 가비지 컬렉터가 let count = 0;을 가져가지 않음! 함수 환경 자체(counter 함수부터 return incre까지)가 가비지컬렉터의 수집 대상이 되지 않음!
// counter 함수가 계속 살아 있는지 안다!
// 결국 let count = 0은 incre 함수만 접근할 수 있음!
let result = counter();
// counter를 실행하면 함수 본문이 나옴 .
// 가비지컬렉터의 수집 대상이 되지 않음..!

// result 실행 -> incre함수 실행됨.
